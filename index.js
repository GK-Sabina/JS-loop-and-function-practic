...
const MIN_VALUE = 1;
const MAX_VALUE = 100;
const secretNumber = getRandomIntInclusive(MIN_VALUE, MAX_VALUE);
let tries = 0;

while(true){
    const userInput = prompt(`Guess the number from ${MIN_VALUE} to ${MAX_VALUE}`);
    ...
    if (userInput < MIN_VALUE || userInput > MAX_VALUE) {
        alert(`Number should be between ${MIN_VALUE} and ${MAX_VALUE}`);
        continue;
    }
    ...
    if (Number(userInput) === secretNumber) {
        alert(`You won! Count of tries: ${tries}`);
        break;
    }
